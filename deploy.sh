...
mkdir -p ${tmp_dir}
cp ci-common/cleanWorkspace.py ${tmp_dir}/cleanWorkspace.py
python ${tmp_dir}/cleanWorkspace.py --git_repo_url "${bamboo_gaas_example_lambda_git_url}"

cd "${bamboo_build_working_directory}"
git submodule update --init --recursive --remote
source ci-common/functionsSource.sh

example_lambda_deploy